...
if __name__ == "__main__":
    ...
    yelabels = [yelabel(notelist,i) for i in notelist]
    colors = [getcolor(i) for i in notelist]
                ...
                for betaf in betafiles:
                    for note in notelist:
                        ...
                        ax1.semilogx(x, frac.iloc[:, i], label=get_channel_label(
                            i), linestyle=get_style(i), color=getcolor(note), alpha=0.94)
                        i = 5  # spf
                    ...
                    plt.tight_layout()
                    print colors
                    h = [plt.plot([], [], colors[i], ls='-', alpha=0.5)[0] for i in range(8)]
                    plt.subplots_adjust(left = 0.1,right = 0.95,hspace=0.03,top = 0.89,bottom=0.11)     # the top of the subplots of the figure
                    plt.legend(handles=h, labels=yelabels, loc='upper center',bbox_to_anchor=(0.5, 3.53), 
                            fontsize=11, ncol=4, fancybox=False, shadow=False)
                    plt.savefig('./figs/frac'+fissionfile+masslist[massmodel]+'.eps',format='eps',dpi=1000,rasterized=True)
                    plt.close()
